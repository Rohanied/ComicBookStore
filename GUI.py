...
def borrow_in():
	...
	buy_button=Button(borrow_menu,text="Buy",command=buy_book,font=("Times new roman","10","bold"),bg= "light pink")
	view_image_button=Button(borrow_menu,text="View Cover",command=view_cover,font=("Times new roman","10","bold"),bg= "light green")


	borrow_label1.grid(row=0,columnspan=20)
	borrow_label.grid(row=6,column=0)
	borrow_label2.grid(row=6,column=1)
	borrow_label3.grid(row=6,column=4)
	borrow_label4.grid(row=6,column=7)

	borrow_entry1.grid(row=1,columnspan=20)
	borrow_button1.grid(row=2,columnspan=20)
	buy_button.grid(row=3,columnspan=20)
	view_image_button.grid(row=4,column=20)
	borrow_list.grid(row=7,column=0)
...
def buy_book():
	...
	if pos == -1:
		...
		if(w2[3] == 'Y'):
			...
			f2.write(l3)
			f2.close()
			tkinter.messagebox.showinfo("Purchased","You have successfully purchased "+w2[1]+" book" )
